...
//	echo $dept_id . $dept_name. "<br>";
		...
		$querytotalcat = "SELECT ifnull(title,'Not Specified') as title, coalesce(sum(total),0)  as total_amount,  coalesce(sum(qty),0) as total_qty 
		FROM 94_total_byprod
		WHERE (id = ".$uid.")";
...
//		echo "<br>".$querytotalcat."<br>";
		...
		$sqlupdate = "update 94_prodserv set graph_qty = ".$total_qty.", graph_total = ".$total_total. " where uid = ".$uid;
		...
		$sqlstaff2 = "SELECT * from 94_prodserv  order by graph_qty desc";
	//				echo $sqlstaff2;
		...
		if ($resultstaff2->num_rows > 0) {
			...
			while($rowstvehicle2 = $resultstaff2->fetch_assoc()) {
				...
				$sqlupdate2 = "update 94_prodserv set graph_sel = ".$graph_sel." where uid = ".$uid;
//	echo $sqlupdate2;
			...
			if (strlen($graph_colour) == 0){
				...
				$sqlupdate2c = "update 94_prodserv set graph_colour = '".$colours[$ic] ."' where uid = ".$uid;
